    ...
    public partial struct HeatSystem : ISystem {
        ...
        public void OnUpdate(ref SystemState state) {
            ...
            // update the colors and heights of the ground cells from the heat data
            {
                ...
                // state.Dependency = GroundCellUpdate_SingleThreadJob(state.Dependency, ref state, heatBuffer, config);
        ...
        private float Index(int row, int col, NativeArray<Heat> heatRO, int numColumns, int numRows) {
            if (col < 0 || col >= numColumns || row < 0 || row >= numRows) return 0;

            return heatRO[row * numColumns + col].Value;
        }

        private JobHandle HeatSpread_SingleThreadedJob(JobHandle dependency, ref SystemState state, DynamicBuffer<Heat> heatBuffer, Config config) {
        ...
        private JobHandle HeatSpread_ParallelJob(JobHandle dependency, ref SystemState state, DynamicBuffer<Heat> heatBuffer, Config config) {
        ...
        private void GroundCellUpdate_MainThread(ref SystemState state, DynamicBuffer<Heat> heatBuffer, Config config) {
            ...
            var elapsedTime = (float)SystemAPI.Time.DeltaTime;

            foreach (var (trans, color) in SystemAPI.Query<RefRW<LocalTransform>, RefRW<URPMaterialPropertyBaseColor>>().WithAll<GroundCell>()) {
        ...
        private JobHandle GroundCellUpdate_SingleThreadJob(JobHandle dependency, ref SystemState state, DynamicBuffer<Heat> heatBuffer, Config config) {
            var minY = -(config.GroundCellYScale / 2);
            var groundCellUpdateJob = new GroundCellUpdate {
                Config = config,
                HeatBuffer = heatBuffer,
                ElapseTime = (float)SystemAPI.Time.DeltaTime,
                MinY = minY,
        ...
        private JobHandle GroundCellUpdate_ParallelJob(JobHandle dependency, ref SystemState state, DynamicBuffer<Heat> heatBuffer, Config config) {
            var minY = -(config.GroundCellYScale / 2);
            var groundCellUpdateJob = new GroundCellUpdate {
                Config = config,
                HeatBuffer = heatBuffer,
        ...
        public partial struct GroundCellUpdate : IJobEntity {
            ...
            public void Execute(ref LocalTransform trans, ref URPMaterialPropertyBaseColor color, [EntityIndexInQuery] int entityIdx) {
                ...
                // oscillate the displayed heat so that the fire looks a little more organic
                {
                    var radians = Random.CreateFromIndex((uint)entityIdx).NextFloat(math.PI * 2) + ElapseTime;
                    var oscillationOffset = math.sin(radians) * heat * Config.HeatOscillationScale; // the more heat, the more oscillation
                    heat += oscillationOffset;
                }
