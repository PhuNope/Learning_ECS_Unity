...
namespace Firefighters.Step_3 {
    public partial struct BucketSystem : ISystem {
        ...
        public void OnUpdate(ref SystemState state) {
            ...
            foreach (var (bucket, trans, color) in SystemAPI.Query<RefRW<Bucket>, RefRW<LocalTransform>, RefRW<URPMaterialPropertyBaseColor>>()) {
                // we only really need to update the color when the water value changes
