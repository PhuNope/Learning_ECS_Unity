...
namespace Firefighters.Step_3 {
    public partial struct BotSystem : ISystem {
        ...
        private void BotUpdate_MainThread(ref SystemState state, DynamicBuffer<Heat> heatBuffer, float moveSpeed, float fillRate, int numRows, int numCols) {
        ...
        private static bool MoveToTarget(ref LocalTransform botTrans, float2 targetPos, float moveSpeed) {
            ...
            // the animated model face up the z axis, so need to rotate it 90 degrees clockwise
            var modelRotation = math.radians(90);

            // atan2 returns a counter-clockwise angle of rotation, so negate to make it clockwise
