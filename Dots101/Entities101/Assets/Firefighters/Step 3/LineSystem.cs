...
namespace Firefighters.Step_3 {
    public partial struct LineSystem : ISystem {
        ...
        public void OnUpdate(ref SystemState state) {
            ...
            var rand = new Random(123 + seed++); // seed is incremented to get different random value in different frames
            ...
            // The Query will only math entities whose RepositionLine is enabled.
            foreach (var (team, members, repositionLineState) in SystemAPI.Query<RefRO<Team>, DynamicBuffer<TeamMember>, EnabledRefRW<RepositionLine>>()) {
                
                repositionLineState.ValueRW = false; // disable RepositionLine
                ...
                // set LinePos of the team's bots and set their bot state
                {
                    ...
                    var nearestFirePos = HeatSystem.NearestFire(randomPondPos, heatBuffer, config.GroundNumRows, config.GroundNumColumns, config.HeatDouseTargetMin);
