...
namespace Tutorials.Kickball.Step_2 {
    public partial struct PlayerMovementSystem : ISystem {
        ...
        public void OnUpdate(ref SystemState state) {
            ...
            // Get directional input. (Most of the UnityEngine.Input are Brust compatible, but not all are).
            // If the OnUpdate, OnCreate, or OnDestroy methods needs to access managed objects or call methods that aren't Burst-compatible, the [BurstCompile] attribute can be omitted.
            var horizontal = Input.GetAxis("Horizontal");
            var vertical = Input.GetAxis("Vertical");
            var input = new float3(horizontal, 0, vertical) * SystemAPI.Time.DeltaTime * config.PlayerSpeed;

            // If there's no directional input this frame, don't need to move the players.
            if (input.Equals(float3.zero)) return;

            var minDist = config.ObstacleRadius + 0.5f; // the player capsule radius is 0.5f
            var minDistSQ = minDist * minDist;

            // For every entity having a LocalTransform and Player component, a read-write reference to the LocalTransform is assigned to 'playerTransform'
