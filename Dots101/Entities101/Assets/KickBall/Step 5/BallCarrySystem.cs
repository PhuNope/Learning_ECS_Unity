...
using Tutorials.KickBall.Step_3;
using Unity.Burst;
...
namespace Tutorials.KickBall.Step_5 {
    [UpdateBefore(typeof(BallMovementSystem))]
    public partial struct BallCarrySystem : ISystem {
        private static readonly float3 CarryOffset = new float3(0, 2, 0);
        ...
        public void OnUpdate(ref SystemState state) {
            ...
            foreach (var (playerTransform, playerEntity) in SystemAPI.Query<RefRW<LocalTransform>>().WithAll<Player>().WithEntityAccess()) {
                if (state.EntityManager.IsComponentEnabled<Carry>(playerEntity)) {
                    ...
                    foreach (var (ballTransform, ballEntity) in SystemAPI.Query<RefRO<LocalTransform>>().WithAll<Ball>().WithDisabled<Carry>().WithEntityAccess()) {
