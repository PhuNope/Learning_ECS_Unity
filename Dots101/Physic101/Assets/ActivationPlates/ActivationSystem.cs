...
namespace ActivationPlates {
    // the system run after collision detection and the solver
        ...
        public void OnUpdate(ref SystemState state) {
            ...
            // for zones that triggered events this update, set their state to Inside or Enter
            {
                ...
                foreach (var triggerEvent in sim.TriggerEvents) {
                    ...
                    if (SystemAPI.HasComponent<Player>(triggerEvent.EntityA) && SystemAPI.HasComponent<Zone>(triggerEvent.EntityB)) {
                        playerEntity = triggerEvent.EntityA;
                        zoneEntity = triggerEvent.EntityB;
                    }
                    else if (SystemAPI.HasComponent<Player>(triggerEvent.EntityB) && SystemAPI.HasComponent<Zone>(triggerEvent.EntityA)) {
                    ...
                    else if (zone.ValueRO.State == ZoneState.Exit || zone.ValueRO.State == ZoneState.Outside) {
            ...
            {
                foreach (var zone in SystemAPI.Query<RefRW<Zone>>()) {
            ...
            // set color of the zones to green when entered, red when exit
            {
                foreach (var (zone, color) in SystemAPI.Query<RefRW<Zone>, RefRW<URPMaterialPropertyBaseColor>>()) {
                    if (zone.ValueRO.State == ZoneState.Enter) {
